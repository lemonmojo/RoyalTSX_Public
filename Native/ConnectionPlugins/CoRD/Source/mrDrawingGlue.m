...
{
	return [[mrBitmap alloc] initWithBitmapData:data size:NSMakeSize(width, height) view:conn->ui];
}

void ui_paint_bitmap(RDConnectionRef conn, int x, int y, int cx, int cy, int width, int height, uint8 * data)
{
	mrBitmap *bitmap = [[mrBitmap alloc] initWithBitmapData:data size:NSMakeSize(width, height) view:conn->ui];
...
void ui_desktop_restore(RDConnectionRef conn, uint32 offset, int x, int y, int w, int h)
{
	...
	mrBitmap *b = [[mrBitmap alloc] initWithBitmapData:(const unsigned char *)data size:NSMakeSize(w, h) view:v];
...
static void schedule_display_in_rect(RDConnectionRef conn, NSRect r)
{
	// Since mrSessionView simply flushes its entire buffer to view on draw, it's simpler and quicker to simply update the entire view. This remains from the non-OpenGL implementation but may be useful in the future.
	conn->updateEntireScreen = YES;
}
...
{
	return [[mrBitmap alloc] initWithGlyphData:data size:NSMakeSize(width, height) view:conn->ui];
}
...
void ui_drawglyph(RDConnectionRef conn, int x, int y, int w, int h, mrBitmap *glyph, NSColor *fgcolor, NSColor *bgcolor);
...
{
	return [[mrBitmap alloc] initWithCursorData:xormask alpha:andmask size:NSMakeSize(width, height) hotspot:NSMakePoint(x, y) view:conn->ui bpp:bpp];
}

void ui_set_null_cursor(RDConnectionRef conn)
{
	static mrBitmap *nullCursor = nil;
...
void ui_move_pointer(RDConnectionRef conn, int x, int y)
{
	...
	/* this is incorrect; not sure why this should be implemented
     ...
     mrLog(mrLogLevelInfo, @"Setting point to remote (%d, %d), or local screen %@", x, y, NSStringFromPoint(NSMakePoint(windowPoint.x+windowOrigin.x, windowPoint.y+windowPoint.y)));
     CGWarpMouseCursorPosition(CGPointMake(windowPoint.x+windowOrigin.x, windowPoint.y+windowPoint.y));
     */
	//mrLog(mrLogLevelInfo, @"Should move mouse to %d, %d", x, y);
}
