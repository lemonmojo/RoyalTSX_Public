...
#define GET_MODIFIER_FLAGS(f) (mrPreferenceIsEnabled(mrPrefsIgnoreCustomModifiers) ? [mrSwappedModifiersUtility physicalModifiersForVirtualFlags:f] : f )


static NSDictionary *windowsKeymapTable = nil;

@interface mrKeyboard (Private)
- (BOOL)readKeymap;
...
{
	uint16 rdflags = [mrKeyboard modifiersForEvent:ev];
...
{
	if ( ((scancode == SCANCODE_CHAR_LWIN) || (scancode == SCANCODE_CHAR_RWIN)) && !mrPreferenceIsEnabled(mrDefaultsSendWindowsKey))
		return;
...
- (BOOL)readKeymap
{
    NSString *filePath = [[NSBundle bundleForClass:[self class]] pathForResource:@"keymap" ofType:@"txt"];
	...
	for (NSString *line in fileLines)
	{
		...
		if (!b)
			mrLog(mrLogLevelError, @"Reading Keymap - Uncaught keymap syntax error at line %d. Ignoring.", lineNumber - 1);
	...
	{
		case kKeyboardISO:
			mrLog(mrLogLevelDebug, @"Enabling hacks for European keyboard");
...
{
	mrLog(mrLogLevelInfo, @"Loading windows keymap for Mac keymap identifier %@", keymapIdentifier);
	...
	if (windowsKeymapTable == nil)
	{
		...
		NSString *filename = [[NSBundle bundleForClass:[self class]] pathForResource:@"windows_keymap_table" ofType:@"txt"];
	...
	if (!windowsKeymap)
	{
		...
		for (NSString *potentialKeymapName in windowsKeymapTable)
			...
			{ 
				...
				mrLog(mrLogLevelDebug, @"windowsKeymapForMacKeymap: substituting keymap '%@' for passed '%@', giving Windows keymap 0x%x", potentialKeymapName, keymapIdentifier, [windowsKeymap intValue]);
				break;
			}
	}
	
	if (windowsKeymap)
		mrLog(mrLogLevelInfo, @"Setting remote keymap to %@ (int value: 0x%x)", keymapIdentifier, [windowsKeymap unsignedIntValue]);
	else
		mrLog(mrLogLevelInfo, @"Using default American English keyboard layout");
...
+ (NSString *)currentKeymapIdentifier
{
	...
	mrLog(mrLogLevelDebug, @"Current keymap identifier: %@", (NSString *)TISGetInputSourceProperty(keyLayout, kTISPropertyInputSourceID));
