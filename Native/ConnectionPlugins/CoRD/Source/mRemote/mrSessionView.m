...
@interface mrSessionView (Private)
...
- (id)initWithFrame:(NSRect)frame
{
	...
	keyTranslator = [[mrKeyboard alloc] init];
...
- (void)reshape
{
	...
	if ( ([controller status] != mrConnectionConnected) || [[NSThread currentThread] isEqualTo:[controller valueForKey:@"connectionThread"]] ) 
		return;
...
- (void)mouseMoved:(id)ev
{
	...
	@synchronized(self)
	{
		...
		if ([[NSDate date] timeIntervalSinceDate:lastMouseEventSentAt] >= (1.0f/mrMouseEventLimit))
		{
		...
		else if ([ev isKindOfClass:[NSEvent class]])
		{
			...
			mouseInputScheduler = [[NSTimer scheduledTimerWithTimeInterval:((1.0/mrMouseEventLimit)-[[NSDate date] timeIntervalSinceDate:lastMouseEventSentAt]) target:self selector:@selector(recheckScheduledMouseInput:) userInfo:nil repeats:NO] retain];
		}
	}
}
...
- (void)drawBitmap:(mrBitmap *)image inRect:(NSRect)to from:(NSPoint)origin operation:(NSCompositingOperation)op
{
	[self focusBackingStore];
...
- (void)drawGlyph:(mrBitmap *)glyph at:(NSRect)r foregroundColor:(NSColor *)foregroundColor;
{
...
- (void)resetClip
{
	clipRect = mrRectFromSize(screenSize);
}
...
- (NSImage*)getScreenCapture
{
    ...
    NSImage *bmp = [[NSImage alloc] initWithData:[img representationUsingType:NSPNGFileType properties:nil]];
    [img release];
...
- (void)setScreenSize:(NSSize)newSize
{
	...
	screenSize = newSize; 
	[self setBounds:mrRectFromSize(screenSize)];
...
- (void)setController:(mrSession *)instance
{
	controller = instance;
