...
{
	mrLog(mrLogLevelInfo, @"Adding IO-req for fd %d", file);
...
{
	mrLog(mrLogLevelInfo, @"Removing IO-req for fd %d", fd);
...
{
	mrLog(mrLogLevelInfo, @"Data became available for fd %d", file);
	...
	if (iorq == NULL)
	{
		mrLog(mrLogLevelInfo, @"Couldn't find a matching iorq for file %u", file);
		return;
	}	
